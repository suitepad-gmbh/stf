    ...
    .define(function (options, adb) {
      adb.listDevices()
        .then(function (devices) {
          ...
          if (isFound) {
            adb.shell(serial, 'am broadcast -a de.suitepad.app.kiosk -eaction exit')
              .timeout(5000)
              .then(function (out) {
                res.status(200).json({
                  success: true,
    ...
    .define(function (options, adb) {
      adb.listDevices()
        .then(function (devices) {
          var isFound = devices.filter(device => device.id === serial).length === 1
          if (isFound) {
            adb.shell(serial, 'am broadcast -a de.suitepad.app.kiosk -eaction enter')
              .timeout(5000)
              .then(function (out) {
                res.status(200).json({
                  success: true,
...
function installApk(req, res) {
  const serials = req.swagger.params.serials.value.split(',')
    .map(item => item.trim())
    .filter(item => item)
  if (!serials) {
    res.status(400).json({
      success: false,
      message: 'Missing serials'
    })
    return
  }
  log.info("trying to install apk on: " + serials)
  dbapi.loadDevices(req.user.groups.subscribed)
    .then(function (devices) {
      var targetDevices = serials.map(serial => _.filter(devices, { 'serial': serial })[0])
      for (var i in targetDevices) {
        if (!targetDevices[i]) {
          throw Error('Device ' + serials[i] + ' not found')
        }
      }
      syrup.serial()
        .dependency(require('../adb'))
        .define(function (options, adb) {
          installApkThroughAdb(createStream(req), res, adb, targetDevices)
        })
        .consume()
        .catch(function (err) {
          log.error(err)
...
function createStream(req) {
  ...
  stream.put(req.files.apk.buffer)
  stream.stop()
  return stream
}

function installApkThroughAdb(stream, res, adb, targetDevices) {
  adb.listDevices()
    .then(function (allAdbDevices) {
      log.info("adb devices")
      log.info(allAdbDevices)
      var adbDevices = targetDevices.map(device => allAdbDevices.filter(adbDevice => adbDevice.id === device.serial)[0])
      for (var i in adbDevices) {
        if (!adbDevices[i]) {
          throw new Error("Adb access to " + targetDevices[i].serial + " unavailable")
        }
      }

      var Passthrough = require('stream').PassThrough
      var streams = adbDevices.map(function (device) {
      ...
      return Promise.map(streams, function (adbStream) {
        log.info("Installing apk now on " + adbStream.device.id)
        return adb.install(adbStream.device.id, adbStream.stream)
      })
    })
    .then(function () {
      res.status(200).json({
